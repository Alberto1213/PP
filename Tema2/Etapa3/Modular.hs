...
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f xs = apply f xs 0 (length xs)
    where 
        apply f xs i limit = if (i==limit) then [] else ((take i xs) ++ [(f (xs!!i))] ++ (drop (i+1) xs)) : (apply f xs (i+1) limit)
...
partitions :: [a] -> [[[a]]]
partitions (x:[]) = [[[x]]]
partitions (x:xs) = (map ([x]:) (partitions xs)) ++ foldr (++) [] [(mapSingle ([x]++) y) | y<-(partitions xs)]
 --baga x ul separat in partitie si restul elementelor submultimi fara x 
 --foldr pt fiecare partitie veche:bag x in fiecare element (submulțime) al partitiei respective pe rand 
...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph = S.size (S.map (\x -> S.difference x set) ((S.map (\x -> outNeighbors x graph) set))) == 1 &&
                    S.size (S.map (\x -> S.difference x set) ((S.map (\x -> inNeighbors x graph) set))) == 1 

                    --Pentru un nod scoate multimea outNeighbors;
                    --din aceasta multime scoate nodurile din presupusul modul 
                    --avem multimea mutimilor de vecini 
                    --pt ca Set sa fie modul trebuie ca multimea multimilor de vecini sa aiba un singur elem 
{-
    *** TODO ***
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = foldr (&&) True (S.map (\x -> isModule x graph) partition) --verifica daca toate multimile din 
                                                                                                --partitie sunt module
...
maximalModularPartition partitions graph = Data.List.minimumBy (compare `on` length) (filter (\x -> isModularPartition x graph && length x > 1) partitions)
