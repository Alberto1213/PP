...
(define (key n)
  (let ([x (caddr (get-nth-quadruple n))])
    (let ([x (cadr (get-nth-quadruple n))]
          [y x])
      (map(lambda (func)
                   (if (< (remainder (func x y) 27) 0)
                       (+ (remainder (func x y) 27) 27)
                       (remainder (func x y) 27)))
...
(define (codes->message codes)
  (list->string(map (lambda (f)
        (if (eq? f 0)
             (integer->char 32)
             (integer->char (+ f 96))))codes)))
...
(define (special-func key size) (let lista ([lst key] [n size])
    (if (< n 0)
        '()
        (append lst (lista lst(- n (length lst)))))))
(define (extend-key key size)
 (if (> (length (special-func key size)) size)
     (cut-last (special-func key size) (- (length (special-func key size)) size))
     (special-func key size)
  ))
(define (cut-last list nr)
  (if( > nr 0)(cut-last (reverse(cdr(reverse list))) (- nr 1))
     list))
...
(define (encrypt-codes message key)
  (map (lambda (f t) (remainder (+ f t) 27)) message (extend-key key (length message))))
(define (decrypt-codes crypted key)
   (map (lambda (f t)
          (if(< (remainder (- f t) 27) 0)
             (+ (remainder (- f t) 27) 27)
             (remainder (- f t) 27))
          ) crypted (extend-key key (length crypted))))
...
(define (encrypt-message message key)
  (codes->message (encrypt-codes (message->codes message) (extend-key key (length (message->codes message))))))
(define (decrypt-message message key)
  (codes->message (decrypt-codes (message->codes message) (extend-key key (length (message->codes message))))))
